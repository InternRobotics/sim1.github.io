...
const fitParams = {
    'Real Data': { a: -0.30367, b: 1.682, c: -1.3743, epsilon: 0.03823 },
    'Sim Teleoperated Data': { a: -0.13965, b: 1.1160, c: -0.9656, epsilon: 0.0050665 },
    'Sim Generated Data': { a: 0.16900, b: -0.4799, c: 0.2720, epsilon: 0.12878 }
};
...
function createChart() {
    ...
    if (typeof window !== 'undefined') {
        if (window.__resultsChartReplayTimeoutId) {
            clearTimeout(window.__resultsChartReplayTimeoutId);
            window.__resultsChartReplayTimeoutId = null;
        }
    }
    ...
    // Own tooltip id — results-chart-2 also uses .chart-tooltip; selectAll().remove() was deleting the sibling chart's node and breaking position.
    d3.select('#results-chart-tooltip').remove();
    const tipOffX = 12;
    const tipOffY = 14;

    const tooltip = d3.select('body')
        .append('div')
        .attr('id', 'results-chart-tooltip')
    ...
    // Draw auxiliary lines - horizontal then vertical
    const realDataset = datasets.find(
        d => d.label === 'Real' || d.label === 'Real Data'
    );
    const genDataset = datasets.find(
        d => d.label === 'Gen' || d.label === 'Sim Generated Data'
    );
    
    if (realDataset && genDataset) {
        const realVertex = calculateVertex(realDataset);
        if (realVertex && realVertex.y <= 1 && realVertex.y >= 0) {
            // Keep Chart 1 auxiliary intersection fixed at x=7500.
            // (Do not use approximated solve result like 7534.)
            const xLeft = 500;
            const xRight = 7500;

                // Calculate when all fit curves finish animating
                // Last curve (Gen) starts at: 2 * 500 + (8 * 200 + 500) = 1000 + 2100 = 3100
                // Last curve animation duration: 1000ms
                // So all curves finish at: 3100 + 1000 = 4100ms
                ...
                const auxiliaryDelay = allCurvesFinish + 500; // Wait 500ms after all curves finish
                
                // First vertical line at x=500 (Real vertex position) from curve to x-axis
                g.append('line')
                    .attr('class', 'auxiliary-line vertical-start')
                    .attr('x1', xScale(xLeft))
                    .attr('y1', yScale(realVertex.y))
                ...
                // Horizontal line from Real vertex to Gen curve - same time as first vertical
                g.append('line')
                    .attr('class', 'auxiliary-line horizontal')
                    .attr('x1', xScale(xLeft))
                    .attr('y1', yScale(realVertex.y))
                    .attr('x2', xScale(xRight))
                    .attr('y2', yScale(realVertex.y))
                    .attr('stroke', annotationColor)
                    .attr('stroke-width', 2)
                    .attr('stroke-dasharray', '5,5')
                    .attr('opacity', 0)
                ...
                // Second vertical line at x=7500 (intersection point) from curve to x-axis - same time
                g.append('line')
                ...
                // Label for Real vertex x-coordinate (500) - after all lines
                g.append('text')
                    .attr('class', 'auxiliary-label')
                    .attr('x', xScale(xLeft))
                    .attr('y', yScale(realVertex.y) - 15)
                    .attr('text-anchor', 'middle')
                    .style('font-size', '14px')
                    .style('fill', annotationColor)
                ...
                // Label for Gen intersection x-coordinate (7500) - same time as 500
                g.append('text')
                    .attr('class', 'auxiliary-label')
                    .attr('x', xScale(xRight) + 10)
                    .attr('y', yScale(realVertex.y))
    ...
    if (realDataset && genDataset) {
        const realVertex = calculateVertex(realDataset);
        // Keep finish-time logic consistent with drawing logic (which now has xRight fallback).
        if (realVertex && realVertex.y <= 1 && realVertex.y >= 0) {
    ...
    if (typeof window !== 'undefined') {
        window.__resultsChartReplayTimeoutId = setTimeout(() => {
            // ensure we stop any in-flight transitions before recreating
            try { d3.select('#chart-container').selectAll('*').interrupt(); } catch (_) {}
            createChart();
